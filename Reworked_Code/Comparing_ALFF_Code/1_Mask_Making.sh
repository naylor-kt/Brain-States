...
### Create binarised versions of the masks
hemi=(lh rh)

for r in `cat ${mask_path}/Region_List.txt`; do

    fslmaths ${mask_path}/Extracted_Region/HarvOx-${r} -bin ${mask_path}/Extracted_Region/binarised/HarvOx-${r}-bin.nii.gz

    for h in ${hemi[@]}; do
    
        fslmaths ${mask_path}/Extracted_Region/${h}/HarvOx-${r}-${h} -bin ${mask_path}/Extracted_Region/${h}/binarised/HarvOx-${r}-${h}-bin.nii.gz

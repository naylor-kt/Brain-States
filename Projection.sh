...
mkdir -p ${data_path}/Freesurfer/Registration/$s/Projected/Temp_Filt_025/
    ...
    for c in ${cond[@]}; do
        ...
        mri_vol2surf --mov ${data_path}/Percent_Signal_Change/Temp_Filt_025/${s}/${s}-${c}-tf025-psc.nii.gz \
             --hemi ${h} \
             --o ${fs_path}/Registration/$s/Projected/Temp_Filt_025/${s}-${c}-${h}-tf025-fsavg.mgz \
             --projfrac-avg 0 1 0.1 \
             --reg ${fs_path}/Registration/$s/${s}-${c}_mean2fs.lta \
             --srcsubject ${s}
        done
        ...
        for h in ${hemi[@]}; do
        mris_apply_reg --src ${fs_path}/Registration/$s/Projected/Temp_Filt_025/${s}-${c}-${h}-tf025-fsavg.mgz \
                   --o ${fs_path}/Registration/$s/Projected/Temp_Filt_025/${s}-${c}-${h}-tf025-fsavg.mgz \
                   --streg $SUBJECTS_DIR/${s}/surf/${h}.fsaverage.sphere.reg $SUBJECTS_DIR/fsaverage/surf/${h}.sphere.reg
        done
